...
def label_periodo(fecha_inicio, fecha_fin):
    ...
    return (f"{fecha_inicio.strftime('%d de %B de %Y')} "
            f"al {fecha_fin.strftime('%d de %B de %Y')}")


def detalle_periodo(fecha_inicio, fecha_fin):
    return (f"{fecha_inicio.strftime('%d/%m/%Y')} 00:00"
...
def procesar(nuevos, anteriores, info_mysql=None, info_mongo=None):
    ...
    for item_id, n in nuevos.items():
        ...
        if (peso_new == peso_old and alto_new == alto_old
                and ancho_new == ancho_old and largo_new == largo_old):
            continue

        vol_new = round(ancho_new * alto_new * largo_new / 5000, 2) if all(
            v is not None for v in [ancho_new, alto_new, largo_new]) else None
        vol_old = round(ancho_old * alto_old * largo_old / 5000, 2) if all(
            v is not None for v in [ancho_old, alto_old, largo_old]) else None
        ...
        registros.append({
            ...
            "skus_hijos":     mysql.get("skus_hijos", []),
            # MongoDB
