...
def _hoja_resumen(wb, registros, periodo_det):
    ...
    leyendas = [
        ("ALTA",  "Cambio >= 30% en peso facturable -> Reclamar o ajustar urgente", "FF4D6D"),
...
_COLS_TEXTO_BASE = {4, 5, 6, 7, 10}   # Cuenta, Proveedor, Marca, SKU GME, Estado
...
def _hoja_detalle(wb, registros, periodo_det):
    ...
    for ri, r in enumerate(registros, start=4):
        ...
        # Hijos: lista con padding hasta max_hijos para que la longitud sea fija
        hijos = r.get("skus_hijos", [])
        hijos_padded = hijos + [None] * (max_hijos - len(hijos))
        ...
        for ci, val in enumerate(fila, start=1):
            ...
            if ci >= _COL_INICIO_HIJOS and val is not None:
                c.fill = fill("FEF3C7") if ri % 2 == 0 else fill("FDF6E3")
                c.alignment = lft()
...
def _hoja_urgente(wb, altas, periodo_det):
    ...
    ws["A1"] = "ITEMS QUE REQUIEREN ACCION INMEDIATA -- Cambio >= 30% en peso facturable"
    ...
    COLS3 = [
        ("Item ID",        20), ("Fecha Cambio",   15), ("Tendencia",      13),
        ("Peso Ant. (g)",  14), ("Peso Nvo. (g)",  14),
        ("Fact. Ant. (g)", 14), ("Fact. Nvo. (g)", 14),
        ("% Cambio Fact.", 15), ("Vol. Ant. (kg)",  13), ("Vol. Nvo. (kg)", 13),
    ]
    ...
    for ri, r in enumerate(altas, start=4):
        ...
        fila3 = [
            r["item_id"], r["fecha"], tend_lbl,
            r["peso_old"], r["peso_new"],
            r["fact_old"], r["fact_new"],
            pf,
            r["vol_old"],  r["vol_new"],
        ]
